...
void drawFullHand(){
   ...
   glPushMatrix();
       ...
       //glRotatef(30,0,1,0);
        ...
        //glTranslatef(100*sin(fullHand_angle),0,0);

       // glTranslatef(0.0,0,-(2*(5*radius)+5*(radius-4)));
...
void drawWheel()
{
     ...
     glBegin( GL_TRIANGLE_FAN );
            ...
            // This draws half the circle
            int tempX = -50;
            while ( tempX <= 50 )
            {
                glVertex2d( tempX, sqrt( 50*50 - tempX*tempX ) );
                tempX++;
            }

            // This draws the other half
            tempX = -50;
            while ( tempX <= 50 )
            {
                glVertex2d( tempX, -sqrt( 50*50 - tempX*tempX ) );
